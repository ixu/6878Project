$.fn.clickSlide=function(pagesDiv){
    var width=$(pagesDiv).width(),
        pages=$(pagesDiv).children().css({width:width,minWidth:width})
    pages.hide().eq(0).show()
    $(pagesDiv).width(width*2)
    return this.click(function(){
        var $visible=pages.filter(':visible')
        var $page=$($(this).attr('href'))
        if (!$page.is(':visible')){
            $page.show()
            $(pagesDiv).stop(true,false)
                .css('left',-$visible.position().left)
                .animate({left:-$page.position().left},'slow',function(){
                    $visible.hide()
                    $(this).css('left',0)
                })
        }
...
d3.json("ProjectCode_v1/Output/0.05FPKMAgglomerativeClusters/OutputDirectoryHeirarchicalNoOfClusters_20/geneCluster.json", function(error, root) {
  geneCluster = root;
...
function getGeneDescriptions() {
    d3.text('ProjectCode_v1/Output/0.05FPKMAgglomerativeClusters/OutputDirectoryHeirarchicalNoOfClusters_20/geneDescriptions.txt', 'text/csv', function(text) {
        var genes = d3.csv.parseRows(text);
        for (i=1; i < genes.length; i++) {
...
function updateData() {
    ...
    for (c in currentClusters) {
        ...
        data.push({pathname:c + " average", pathdata: averages[c + " average"], clustername:c + " average"});
    }
    if (maxValue === 0) {
        maxExpression = maxAverageExpression;
        for (average in averages) {
            data.push({pathname:average, pathdata: averages[average], clustername:average});
        }
    } else {
